...
struct GuestText *_guest_cat(const char *path, Error **errp);
...
struct GuestText *_guest_cat(const char *path, Error **errp)
{
    ...
    slog("file-open called, filepath: %s, mode: %s", path, mode);
    fh = safe_open_or_create(path, mode, &local_err);
    if (local_err != NULL) {
        error_propagate(errp, local_err);
        return NULL;
    ...
    slog("file-open, handle: %" PRId64, handle);
    ...
    if (ferror(fh)) {
        ...
        if (read_count) {
            read_data->text = g_base64_encode(buf, read_count);
        }
    }
    ...
    slog("file-close called, handle: %" PRId64, handle);
    if (!gfh) {
        return NULL;
    }
...
{
    return _guest_cat("/proc/meminfo", errp);
}

struct GuestText *qmp_guest_stat(Error **errp)
{
    return _guest_cat("/proc/stat", errp);
}

struct GuestText *qmp_guest_diskstats(Error **errp)
{
    return _guest_cat("/proc/diskstats", errp);
}

struct GuestText *qmp_guest_net_dev(Error **errp)
{
    return _guest_cat("/proc/net/dev", errp);
}

struct GuestText *qmp_guest_loadavg(Error **errp)
{
    return _guest_cat("/proc/loadavg", errp);
}
...
struct GuestText *qmp_guest_df(Error **errp)
{
    ...
    while ((fs = getmntent(fp)) != NULL) {
        if (fs->mnt_fsname[0] != '/') continue;
        if (statfs(fs->mnt_dir, & vfs) != 0) {
            continue;
        }
        ret = bvsprintf(buf+has_read, "%s,%s,%ld,%ld,%ld;", \
            fs->mnt_dir, \
            fs->mnt_fsname, \
            vfs.f_bsize, \   /*every block contain size*/
            vfs.f_blocks, \  /*total block*/
            vfs.f_bfree);    /*free block*/
        has_read += ret;
    }
    endmntent(fp);
    read_data = g_malloc0(sizeof(GuestFileRead));
    if (has_read)
        read_data -> text = g_base64_encode((guchar *)buf, has_read);
