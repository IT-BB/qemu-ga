...
int bvsprintf(char *buffer, const char *format, ...)
{
	va_list aptr;
	int ret;
...
struct GuestText *qmp_guest_meminfo(Error **errp)
{
	GuestText *read_data = NULL;
	char buf[512];
	...
	bvsprintf(buf, "total: %ld, usage: 0.%04ld", statex.ullTotalPhys/1048576, statex.dwMemoryLoad);

	read_data = g_malloc0(sizeof(GuestText));

	read_data -> text = g_base64_encode((guchar *)buf, strlen(buf));

	return read_data;
}

__int64 guest_CompareFileTime(FILETIME time1, FILETIME time2)  
{  
...
__int64  getWinCpuUsage() {  
	...
	hEvent = CreateEventA(NULL, FALSE, FALSE, NULL);
        ...
        __int64 idle = guest_CompareFileTime(preidleTime, idleTime);  
        __int64 kernel = guest_CompareFileTime(prekernelTime, kernelTime);  
        __int64 user = guest_CompareFileTime(preuserTime, userTime);  
...
struct GuestText *qmp_guest_stat(Error **errp)
{
	GuestText *read_data = NULL;
	char buf[512];
	...
	bvsprintf(buf, "usage: 0.%04ld", getWinCpuUsage());
...
/*Modified by: wangxianliang*/
/*only disk C:\ */
struct GuestText *qmp_guest_diskstats(Error **errp)
{
	GuestText *read_data = NULL;
	char buf[512];
	int has_read=0, ret;
	...
	BOOL bResult = FALSE;
    	HANDLE dev = CreateFile(filePath, 
        	GENERIC_READ, 
	...
	read_data = g_malloc0(sizeof(GuestText));
		...
		ret = bvsprintf(buf+has_read, "create file fail\n");
		has_read += ret;	
		read_data -> text = g_base64_encode((guchar *)buf, strlen(buf));
		return read_data;	
    	}

	bResult = DeviceIoControl(dev, 
	...
	ret = bvsprintf(buf+has_read, "%s,%lld,%lld,%lld,%lld,%lld\n",
							"C:\\",
							disk_info.ReadCount,
							disk_info.WriteCount,
							disk_info.BytesRead,
							disk_info.BytesWritten,
							disk_info.SplitCount							
							);		
...
/*Modified by: wangxianliang*/
struct GuestText *qmp_guest_net_dev(Error **errp)
{
	GuestText *read_data = NULL;
	int has_read=0, ret;
	char buf[512];

    MIB_IF_TABLE2 *pMibIfTable = NULL;
	pMibIfTable = (MIB_IF_TABLE2*)malloc(sizeof (MIB_IF_TABLE2));

    GetIfTable2Ex(MibIfTableRaw,&pMibIfTable);
    int i =0;

	for ( ; i != pMibIfTable->NumEntries; ++i){
        if(pMibIfTable->Table[i].OutOctets > 0){
		ret = bvsprintf(buf+has_read, "%S:%lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld:%S\n",
                                                        pMibIfTable->Table[i].Description, 
							pMibIfTable->Table[i].OutOctets ,  
							pMibIfTable->Table[i].InOctets ,
                                                        pMibIfTable->Table[i].OutUcastPkts,
                                                        pMibIfTable->Table[i].InUcastPkts,
                                                        pMibIfTable->Table[i].OutErrors,
                                                        pMibIfTable->Table[i].InErrors,
                                                        pMibIfTable->Table[i].OutDiscards,
                                                        pMibIfTable->Table[i].InDiscards,
							pMibIfTable->Table[i].PhysicalAddress
							);
        has_read += ret;
        }
    }		
	FreeMibTable(pMibIfTable);
	read_data = g_malloc0(sizeof(GuestText));
	read_data -> text = g_base64_encode((guchar *)buf, strlen(buf));
	return read_data;
...
struct GuestText *qmp_guest_df(Error **errp)
{
	...
	for(; nIndex < driver_number; nIndex++)
	{
		if(GetDriveType(lpDrives + nIndex * 4) != 3 ) continue;
		...
		ret = bvsprintf(buf+has_read, "%s,%lld,%lld\n", lpDrives + nIndex * 4,
